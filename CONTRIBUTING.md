...
AI駆動開発において、アーキテクチャの崩壊を防ぐための静的検査の仕組み。
詳細は [guards/README.md](./guards/README.md) を参照。

#### @what / @why / @failure パターン

ガードレールやルールを定義する際は、以下のコメントパターンを使用：
...
これは**AIに意図を伝えるためのコメント**です。AIがルールを修正する際、
「なぜこのルールがあるのか」を理解した上で作業できます。
...
| 構造の憲法 | 関心事の分離 |
| コード品質の憲法 | 曖昧さの排除 |
| デザインの憲法 | 一貫性とアクセシビリティ |
| 保守の憲法 | ボーイスカウトのルール |
| 速度の憲法 | パフォーマンスは機能の一部 |
| UXの憲法 | 認知負荷の最小化 |
| 価値の憲法 | ユーザーエンゲージメントの最大化 |
